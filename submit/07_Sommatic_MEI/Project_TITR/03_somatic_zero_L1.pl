...
while ($line = <LIST>)
   {
    ...
    for $TE(keys %TE)
       {
        $call_file = $path.'/SOM'.$ARGV[2].'_'.$ARGV[3].'/LINE/TITR.LINE.noCONT.'.$cont.'plus.learn.'.$cutoff.'.filter.calls';
        ...
        for ($k=0; $k<=$#sample; $k++)
            {
             if ( ($sample[$k] =~ /CONT\_/) || ($sample[$k] =~ /TITR\_/) )
                {
                 $temp_file = '/home/xwzhu/transfer/titration/Bulk/1702UNHX-0011/'.$sample[$k].'/retro_v41_'.$ARGV[3].'/'.$TE.'/'.$sample[$k].'.'.$TE.'.SR.PE.calls';
                }
             elsif ( ($sample[$k] eq 'Astro_1') || ($sample[$k] eq 'Heart_1') || ($sample[$k] eq 'Neuron_1') )
                {
                 $temp_file = '/home/xwzhu/transfer/BulkSeq/adult/'.$sample[$k].'/retro_v41_'.$ARGV[3].'/'.$TE.'/'.$sample[$k].'.'.$TE.'.SR.PE.calls';
                }
             elsif  ($sample[$k] =~ /bulk/)
                {
                 $temp_file = '/home/xwzhu/transfer/BSM/BSM_bulk/'.$sample[$k].'/retro_v41_'.$ARGV[3].'/'.$TE.'/'.$sample[$k].'.'.$TE.'.SR.PE.calls';
                }
             else
                {
                 $temp_file = '/home/xwzhu/transfer/titration/'.$sample[$k].'/retro_v41_'.$ARGV[3].'/'.$TE.'/'.$sample[$k].'.'.$TE.'.SR.PE.calls';
                }
        ...
        $out_file = $path.'/SOM'.$ARGV[2].'_'.$ARGV[3].'/LINE/TITR.LINE.noCONT.'.$cont.'plus.learn.'.$cutoff.'.matrix';
        (open (OUT, ">$out_file")) || die "cannot open the out file\n";
