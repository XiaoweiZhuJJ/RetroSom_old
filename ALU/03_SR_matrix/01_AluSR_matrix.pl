...
while ($line = <SR>)
   {
    chomp($line);
    @data = split("\t", $line);
    next if ( ($data[0] !~ /^chr[0-9XY]/) || ($data[0] =~ /\_/) || ($data[0] =~ /\-/) || ($data[3] !~ /Alu/) );
...
$count = 0;
print OU "chr\tcord1\tcord2\tread\trefpos1\trefpos2\tpos\tneg\tseg\tsupport\tsr\tpe\tmap\tref\tGGCG\tGAGC\tCC\tTG\tCG\tAT\tYB\tdepth\tmap_len\tmap_size\tmap_ratio\tshort\tend3\tend5\tdirection\tcenter\tanchor\tinsert\tA_pair\tA_insert\tA_mm\tA_MapQ\tA_AS\tA_XS\toldSR\tgap\tpAT\tdist\n";
while ($line = <SR>)
   {
    chomp($line);
    #print "read $count\n";
    $count ++;
    @data = split("\t", $line);
    next if ( ($data[0] !~ /^chr[0-9XY]/) || ($data[0] =~ /\_/) || ($data[0] =~ /\-/) );
    ($r1, @temp) = split(/\:/, $data[4]);
    $data[4] = join(':', @temp);
    $read = $data[4]."\t".$data[3]."\t".$data[6]."\t".$data[11]."\t".$data[12]."\t".$data[13]."\t".$data[14];
    $data[4] = $r1.':'.$data[4];
