...
my $in_file = '/home/xwzhu/levinson/retro_camal2/visual/temp/04_PE.alignment';
my $sr_file = '/home/xwzhu/levinson/retro_camal2/visual/temp/05_SR.alignment';
my $ou_file = '/home/xwzhu/levinson/retro_camal2/visual/'.$ARGV[3].'/'.$ARGV[0];
my $chr     = $ARGV[1];
my $te      = $ARGV[2];
my $hg      = $ARGV[4];
...
my $yellow= $im->colorAllocate(255,0,255);
### make the background transparent and interlaced
# $im->transparent($white);
# $im->interlaced('true');

### plot the human seq line ###
...
for ($i = 0; $i <=$#reads;$i++)
   {
    ...
    if ($direction > 0)
       ...
       {
        $im->filledPolygon($arrow,$yellow);
       }
    $im->dashedLine($x1,$y1,$x3,$y3,$black);
    $x1 += 5;
    $y1 -= 7;
    $im->string(gdGiantFont,$x1,$y1,$string,$black);
   }
